  ...
  if (armOpenCvUpdate) {
    ...
    for (Contour contour : contours) {   
      ...
      if (area > minContourArea && area < maxContourArea) {
        ...
        //add perfect circle at the center of every contour
        //each circle will be big enough to enclose the contour
        ...
        PShape childEllipse = createShape(ELLIPSE, center.x, center.y, newWidth, newHeight);
        childEllipse.setStrokeWeight(1);
        childEllipse.setStroke(color(255, 255, 0));
        childEllipse.setFill(false);
        ps.addChild(childEllipse);
      }
    }
  }
}
